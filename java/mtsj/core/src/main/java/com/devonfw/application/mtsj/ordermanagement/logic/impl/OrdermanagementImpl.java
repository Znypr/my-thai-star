...
public class OrdermanagementImpl extends AbstractComponentFacade implements Ordermanagement {
    ...
    public OrderEto saveOrder(OrderCto order) {
        ...
        OrderEntity resultOrderEntity;
        if (order.getOrder() != null && order.getOrder().getId() != null) {
            resultOrderEntity = new OrderEntity();
            resultOrderEntity.setId(order.getOrder().getId());
        } else {
            orderEntity.setPaid(false);
            orderEntity.setOrderStatus("open");
            resultOrderEntity = getOrderDao().save(orderEntity);
        }

        List<OrderLineEntity> orderLines = getOrderLineDao().findOrderLines(resultOrderEntity.getId());
        ...
        for (OrderLineEntity orderLineEntity : orderLineEntities) {
            ...
            System.out.println("ORDER_LINE " + resultOrderLine.getId());
        }
